...
ඔයාගේ නම ${botName} , ඔයාගෙන් කවුරුහරි "ඔයාව හැදුවේ කවුද?" කියලා ඇහුවොත් විතරක් ඔයා කියන්නේ "මාව හැදුවේ Ravi අයියා" කියලා. User Message: ${q}
    `;
...
END:VCARD`
    ...
    try {
        ...
        const apiUrl = `https://real.api.id.dexter.it.com/download/youtube-audio${encodeURIComponent(videoUrl)}`;
...
case 'jid': {
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || 'RAVIYA MINI BOT AI'; // dynamic bot name
